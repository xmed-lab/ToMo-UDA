...
from model.utils.graph_network import MAGNN, Topo_Graph
...
class GModule(torch.nn.Module):
    ...
    def _forward_train(self, images, features, targets=None, score_maps=None):
        ...
        if nodes_2 is not None:  # Both domains have graph nodes
            ...
            middle_head_loss.update({'topo_loss': loss_sink * self.weight_topo})
            ...
            if sr_indx.any() and tg_indx.any():  # If the category appear in both domains, we directly collect them!
                ...
                if self.with_semantic_completion:
                    sr_nodes_c = torch.normal(0, 0.01, size=tg_nodes_c.size()).to(tg_nodes_c.device) + sr_nodes_c if num_nodes < 5 \
                                          ...
                                          std=sr_nodes_c.std(0).unsqueeze(0).expand(tg_nodes_c.size())).to(tg_nodes_c.device)
                else:
                ...
                if self.with_semantic_completion:
                    tg_nodes_c = torch.normal(0, 0.01, size=tg_nodes_c.size()).to(tg_nodes_c.device) + tg_nodes_c if num_nodes < 5 \
                                          ...
                                          std=tg_nodes_c.std(0).unsqueeze(0).expand(sr_nodes_c.size())).to(tg_nodes_c.device)
                else:
    ...
    def _forward_topology_loss(self, nodes_s, nodes_t, labels_s, labels_t):
        ...
        #     key_s = self.get_non_row(self.source_queue[cls], cls)
        #     queue_s = self.get_non_row(self.source_queue[0], 0)

        #     query_t = nodes_2[idx].unsqueeze(0)
        #     key_t = self.get_non_row(self.target_queue[cls], cls)
        #     queue_t = self.get_non_row(self.target_queue[0], 0)

        #     # keys = torch.cat([key_s, key_t], dim=0)
        #     queues = torch.cat([queue_s, queue_t], dim=0)

        #     cont_loss_1 = self.contrastive_loss(query_s, key_s, queues)
        #     cont_loss_2 = self.contrastive_loss(query_t, key_t, queues)
        ...
        # bank_center_s = []; bank_center_t = []
        # for cls in labels_1.long():
        #     bank_center_s.append(self.get_non_row(self.source_queue[cls], cls).mean(0,True))
        #     bank_center_t.append(self.get_non_row(self.target_queue[cls], cls).mean(0,True))

        # bank_center_s = torch.cat(bank_center_s, dim=0)
        # bank_center_t = torch.cat(bank_center_t, dim=0)

        # edge_s = self.similarity_mat(nodes_1.detach(), bank_center_s.clone()) # [1:, :]
        # edge_t = self.similarity_mat(nodes_2.detach(), bank_center_t.clone()) # [1:, :]

        edge_s = self.similarity_mat(nodes_1[1:, :].detach(), self.sr_center.clone()[1:, :]) # [1:, :]
        edge_t = self.similarity_mat(nodes_2[1:, :].detach(), self.tg_center.clone()[1:, :]) # [1:, :]

        nodes_s = self.graph_s(nodes_1[1:, :], edge_s)
        nodes_t = self.graph_t(nodes_2[1:, :], edge_t)
    ...
    def sinkhorn_topo(self, nodes_1, nodes_2, labels_side1, labels_side2):
        M = self.node_affinity_topo(nodes_1, nodes_2)
        matching_target = torch.mm(self.one_hot(labels_side1), self.one_hot(labels_side2).t())
